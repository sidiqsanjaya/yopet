...
?>
        ...
        <div class="mb-4 mt-3 bg-white rounded-lg flex items-center w-full p-3 shadow-sm border border-gray-200">
            ...
            <input type="search" name="search" @keydown.enter="getImages()" placeholder="Search <?php if($_GET['page'] == "more-adopt"){ ?> Animals" <?php } else { ?> Discussion <?php } ?>" x-model="q" class="w-full pl-4 text-sm border-none outline-none focus:outline-none focus:border-none active:border-none bg-transparent" <?php if(isset($_POST['search'])){echo 'value="'.$_POST['search'].'"'; }?> >
